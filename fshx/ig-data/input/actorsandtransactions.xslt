...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="actorsandtransactions" match="/" mode="actorsandtransactions">
        <xsl:param name="dest" select="'pagecontent/2_actors_and_transactions.md'"/>
        <xsl:result-document href="pagecontent/2_actors_and_transactions.md" method="text">
                            ...
                            <xsl:for-each select="/ig:profile/ig:transaction">
                                <xsl:variable name="arrow">
                                    <xsl:choose>
                                        ...
                                        <xsl:when test="(.//ig:message[@from = $A and @to = $B] or .//ig:message[@from = $B and @to = $A]/ig:result)
                                            and (.//ig:message[@from = $B and @to = $A] or .//ig:message[@from = $A and @to = $B]/ig:result)">
                                        ...
                                        <xsl:when test=".//ig:message[@from = $B and @to = $A] or .//ig:message[@from = $A and @to = $B]/ig:result">
...
&lt;tbody>
                <xsl:for-each select="/ig:profile/ig:actor">
                    ...
                    <xsl:for-each select="ig:requires-tx|ig:supports-tx">
                        <xsl:variable name="tx" select="."/>
...
&lt;tr class="<xsl:value-of select="$rowclass"/>">
                        <xsl:if test="position()=1">
&lt;td rowspan="<xsl:value-of select="count($actor/(ig:requires-tx|ig:supports-tx))"/>">
&lt;a href="actors_and_transactions.html#<xsl:value-of select="translate(lower-case($actor/ig:name),' ', '-')"/>"><xsl:value-of select="$actor/ig:name"/>&lt;/a>
&lt;/td>
                        </xsl:if>
&lt;td>
...
&lt;td align='center'>
                        <xsl:choose>
                            ...
                            <xsl:when test="$tx/local-name() = 'supports-tx' and $tx/ig:on-condition">
                                <xsl:text>C</xsl:text>
<!-- generate superscript for when notes -->
                                        &lt;sup><xsl:value-of select='count(preceding::ig:on-condition) + 1'/>&lt;/sup>
                            </xsl:when>
...
&lt;/table>
        ...
        <xsl:for-each select=".//ig:on-condition">
            <xsl:text>&#xA;</xsl:text>
...
&lt;table border="1" style="border: 1px solid black; border-collapse: collapse">
    ...
    &lt;tbody>
        <xsl:for-each select='/ig:profile/ig:actor'>
            <xsl:variable name='actor' select='.'/>
            <xsl:for-each select="/ig:profile/ig:option[ig:participant/@actor = current()/@id]">
                &lt;tr>
                <xsl:if test="position() = 1">
            ...
            <xsl:if test="count(/ig:profile/ig:option/ig:participant[@actor = $actor/@id])=0">
                &lt;tr>&lt;td><xsl:value-of select="ig:name"/>&lt;/td>
                &lt;td>_No Options Defined_&lt;/td>&lt;/tr>
            </xsl:if>
        </xsl:for-each>
    &lt;/tbody>
&lt;/table>
        ...
        <xsl:for-each select="/ig:profile/ig:option">
            <xsl:text>&#xA;#### </xsl:text>
            <xsl:value-of select="ig:name"/>
            <xsl:text>&#xA;</xsl:text>
            <xsl:apply-templates select="ig:description|ig:overview"/>
            <xsl:text>This option applies to the actors as described below.</xsl:text>
            <xsl:for-each select="ig:participant">
                <xsl:variable name="actor" select="/ig:profile/ig:actor[@id=current()/@actor]"/>
                <xsl:text>&#xA;##### </xsl:text>
