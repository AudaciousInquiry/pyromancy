...
<xsl:for-each select="/ig:profile/ig:content">
    <xsl:text>   </xsl:text><xsl:value-of select="position()"/>. [<xsl:value-of select="ig:name"/>](StructureDefinition-<xsl:value-of select="/ig:profile/@id"/>-<xsl:value-of select="@id"/>.html)
</xsl:for-each>
